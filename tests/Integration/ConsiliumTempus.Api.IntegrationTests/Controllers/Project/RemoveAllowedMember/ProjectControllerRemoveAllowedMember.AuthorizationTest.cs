...
public class ProjectControllerRemoveAllowedMemberFromProjectAuthorizationTest(WebAppFactory factory)
    : BaseIntegrationTest(factory, new ProjectData())
{
    // Workspace Authorization
    [Fact]
    public async Task RemoveAllowedMemberFromProject_WhenIsCollaborator_ShouldReturnSuccessResponse()
    {
        await AssertSuccessfulResponse(ProjectData.Users[0]);
    }

    [Fact]
    public async Task RemoveAllowedMemberFromProject_WhenIsNotCollaborator_ShouldReturnForbiddenResponse()
    {
    ...
    [Fact]
    public async Task RemoveAllowedMemberFromProject_WhenIsOwner_ShouldReturnSuccessResponse()
    {
        await AssertSuccessfulResponse(ProjectData.Users[0]);
    }

    [Fact]
    public async Task RemoveAllowedMemberFromProject_WhenIsNotOwner_ShouldReturnForbiddenResponse()
    {
    ...
    private async Task<HttpResponseMessage> ArrangeAndAct(UserAggregate user, ProjectAggregate? project = null)
    {
        ...
        return await Client.Delete($"api/projects/{request.Id}/Remove-Allowed-Member/{request.AllowedMemberId}");
    }
}