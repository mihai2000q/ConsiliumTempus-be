    ...
    : BaseIntegrationTest(factory, new ProjectSprintData())
{
    ...
    [Fact]
    public async Task MoveStageFromProjectSprint_WhenProjectIsNotPrivate_ShouldReturnSuccessResponse()
    {
        await AssertSuccessfulResponse(ProjectSprintData.Users[4], ProjectSprintData.ProjectSprints[^3]);
    }

    [Fact]
    public async Task MoveStageFromProjectSprint_WhenProjectIsPrivateAndIsAllowedMember_ShouldReturnSuccessResponse()
    {
    ...
    private Task<HttpResponseMessage> ArrangeAndAct(UserAggregate user, ProjectSprintAggregate? sprint = null)
    {
        ...
        return Client.Post("api/projects/sprints/Move-Stage/", request);
    }
}